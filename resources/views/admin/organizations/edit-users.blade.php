        ...
        <div class="min-height-200px">
            ...
            <div class="card-box mb-30">
                <div class="d-flex p-3 justify-content-between">
                    <h4 class="">Edit Users List</h4>
                </div>
                <form action="{{route('organizations.update_users_list',request()->route('id'))}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <ul id="list" class="list-group list-group-flush" style="width: 90%;">
                            ...
                            <li class="list-group-item d-flex align-items-center p-0 py-3">
                                <select class="custom-select col-8 mr-3" name="list[{{$key}}]" value="{{$value->id}}">
                                    ...
                                    <option value="{{$user->id}}" {{$user->id == $value->id ? 'selected' : '' }}>{{$user->name}}</option>
                                    @endforeach
                                </select>
...
<script>
    ...
    function addnewRow() {
        document.getElementById('list').insertAdjacentHTML('beforeend',
            `<li class="list-group-item d-flex align-items-center p-0 py-3">
                <select class="custom-select col-8 mr-3" name="list[${index}]" id="">
                <option value="">Select...</option>
                    ${users.map((usesr)=>'<option value="'+usesr.id+'">'+usesr.name+'</option>')}
                </select>
                <a href="javascript:void(0)" onclick="removeItem(this)" type="buttons" class="btn btn-danger border rounded">Remove</a>
            </li>`
        );
